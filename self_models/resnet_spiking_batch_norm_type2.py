...
class BasicBlock(nn.Module):
	...
	def __init__(self, in_planes, planes, stride, dropout,batch_flag,bias,timesteps):
		...
		self.residual_batch0_layer = SeparatedBatchNorm1d(num_features=planes, max_length=timesteps)
		self.residual_batch0_layer1 = SeparatedBatchNorm1d(num_features=planes, max_length=timesteps)	
		self.residual_batch0_layers = {0:self.residual_batch0_layer,1:self.residual_batch0_layer1}

        

		self.residual_batch3_layer = SeparatedBatchNorm1d(num_features=planes, max_length=timesteps)
		self.residual_batch3_layer1 = SeparatedBatchNorm1d(num_features=planes, max_length=timesteps)
		self.residual_batch3_layers = {0:self.residual_batch3_layer,1:self.residual_batch3_layer1}
	...
	def forward(self, dic):
		...
		mem[pos] 		= getattr(leak, 'l'+str(pos)) *self.residual_batch0_layers[0](mem[pos],t) + self.residual_batch0_layers[1](self.residual[0](inp),t) - rst
		...
		mem[pos+1] 		= getattr(leak, 'l'+str(pos+1))*self.residual_batch3_layers[0](mem[pos+1],t) + self.residual_batch3_layers[1](self.residual[3](out_prev),t) + identity - rst
...
class RESNET_SNN_BATCH_NORM(nn.Module):
	...
	def __init__(self, resnet_name, activation='Linear', labels=10, timesteps=75, leak=1.0, default_threshold=1.0, dropout=0.2, dataset='CIFAR10',batch_flag=False,bias=False):
		...
		self.pre_process_batch0_layer  = SeparatedBatchNorm1d(num_features=64, max_length=self.timesteps)
		self.pre_process_batch0_layer1 = SeparatedBatchNorm1d(num_features=64, max_length=self.timesteps)
		self.pre_process_batch3_layer  = SeparatedBatchNorm1d(num_features=64, max_length=self.timesteps)
		self.pre_process_batch3_layer1 = SeparatedBatchNorm1d(num_features=64, max_length=self.timesteps)
		self.pre_process_batch6_layer  = SeparatedBatchNorm1d(num_features=64, max_length=self.timesteps)
		self.pre_process_batch6_layer1 = SeparatedBatchNorm1d(num_features=64, max_length=self.timesteps)
		self.pre_process_batch0_layers = {0:self.pre_process_batch0_layer,1:self.pre_process_batch0_layer1}
		self.pre_process_batch3_layers = {0:self.pre_process_batch3_layer,1:self.pre_process_batch3_layer1}
		self.pre_process_batch6_layers = {0:self.pre_process_batch6_layer,1:self.pre_process_batch6_layer1}

		self.pre_process_batch_norm = {0:self.pre_process_batch0_layers,3:self.pre_process_batch3_layers,6:self.pre_process_batch6_layers}
		...
		for t in range(self.timesteps):
			...
			for l in range(len(self.pre_process)):
				...
				if isinstance(self.pre_process[l], nn.Conv2d):
					...
					self.mem[l] 	= getattr(self.leak, 'l'+str(l)) *self.pre_process_batch_norm[l][0](self.mem[l],t) + self.pre_process_batch_norm[l][1](self.pre_process[l](out_prev),t)- rst
